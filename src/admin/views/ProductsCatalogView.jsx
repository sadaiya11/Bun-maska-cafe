...
import { getLocalCatalog, loadCatalog, updateCatalogProduct } from '../../services/productCatalog'

const emptyDraft = { title: '', description: '', price: '' }
...
export default function ProductsCatalogView({ onProductsChange }) {
  ...
  const beginEdit = (product) => {
    ...
    setDraft({ title: product.title, description: product.description, price: String(product.variants?.[0]?.price ?? 0) })
    setNotice('')
  }
  ...
  const save = async (product, changes) => {
    ...
    const nextProduct = {
      ...product,
      ...changes,
      variants: product.variants.map((variant, index) => index === 0 ? { ...variant, price: primaryPrice } : variant),
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {notice && <p className={`text-xs ${notice.startsWith('Not saved') ? 'text-rose-400' : 'text-emerald-400'}`}>{notice}</p>}
      </div>
